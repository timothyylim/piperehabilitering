...
Google review content is generated by the Arrakis service
`/home/tim/pipe-reviews`. The source of truth for that service lives in this
repo at `ops/pipe-reviews`.

Deploy:

```bash
cd /Users/tim/repos/piperehabilitering/ops/pipe-reviews
./deploy.sh
```

Secrets:

- `ops/pipe-reviews/.env` contains `GOOGLE_PLACES_API_KEY` and
  `PIPE_REHAB_PLACE_ID`.
- `~/.secrets/arrakis/github_pipe_rehab` is the GitHub deploy key used by the
  container to commit review updates back to this repo.

Use the `arrakis` SSH alias for manual checks:

```bash
ssh arrakis "docker logs pipe-reviews --tail 80"
```
